...
- 重複出題チェック（2026-09-24実施）：`data/exam.json` の全年度を横断検索した結果、裁判員制度を単独の主題として扱う択一式問題は本問（R7問2）のみ。令和3年度第41問（多肢選択式・憲法）に裁判員制度に関する記述があるが、論点（国民審査権訴訟の文脈）が異なるため、本問の各肢の結論に影響する重複はない。
