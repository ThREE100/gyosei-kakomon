# 令和5年度 出題リスト（構成固め用ロードマップ）

全60問中、著作権の都合で**問題1・6・56・58〜60は公式PDFに問題文が掲載されていない**（本アプリの
`data/exam.json`にも同じ理由で未収録。令和7年度は除外4問だったが、令和5年度は問6・56も加わり除外6問）。
以下は実際に記事化できる**54問**の一覧。

形式の記号：**択一式A**＝ア〜オ等の正誤の組合せを選ぶ形式（出題形式Aテンプレート。空欄ア〜エの語句の
組合せを1〜5から選ぶ問21、会話文の下線部ア〜ウの正誤組合せを選ぶ問18を含む、いずれも変則パターン）／
...
出題形式別：多肢選択式 3問／択一式A 17問（うち変則2問）／択一式B 31問／記述式 3問

科目別：行政法 22問／民法 11問／一般知識等 10問／憲法 5問／商法・会社法 5問／基礎法学 1問
...
| 26 | 行政法 | 択一式B | 地方公共団体に対する法律の適用に関する次の説明のうち | [q26-chihoukoukyoudantai-houritsu-tekiyou.md](./q26-chihoukoukyoudantai-houritsu-tekiyou.md) 執筆済み |
| 27 | 民法 | 択一式B | 消滅時効に関する次の記述のうち、民法の規定に照らし、誤っているものはどれか。 | [q27-shoumetsu-jikou.md](./q27-shoumetsu-jikou.md) 執筆済み |
...
| 38 | 商法・会社法 | 択一式B | 株式会社の種類株式に関する次の記述のうち、会社法の規定に照らし | [q38-shurui-kabushiki.md](./q38-shurui-kabushiki.md) |
...
- chosashi-appおよびr6・r7と同様、配点比重の大きい**行政法（22問）**から着手し、次に
  **民法（11問）・一般知識等（10問）**、最後に憲法・商法会社法・基礎法学という順で進める案。
...
## 除外6問（問1・6・56・58〜60）の扱い

著作権の都合で問題文が非掲載のため記事化できないが、正解番号だけは公式サイト（`r5ans.html`相当の
公式発表）で確認できる：問1＝①、問6＝②、問56＝⑤、問58＝②、問59＝①、問60＝①。
正解だけの紹介は学習価値が薄いため、当面はこの6問を記事化ロードマップから除外する
（`data/exam.json`とも整合）。将来、別の適法な情報源で問題文が入手できた場合のみ再検討する。

## 令和5年度・正解の検証（2026-09-25）

著者からアップロードされた公式PDF（`r5_mondai.pdf`）本文と、著者が会話に直接貼り付けた公式正解表
（択一式60問の正解＋記述式3問の正解例）を突き合わせた。**このアプリの`data/exam.json`に収録されている
令和5年度の54問すべてで、択一式・多肢選択式（問41〜43の空欄ア〜エ）とも正解が完全一致**（不一致ゼロ）。

## 記述式（問44〜46）の公式正解例（確認済み・2026-09-25）

著者が会話に直接貼り付けた公式正解表より、各記事の「公式正解例」セクションでそのまま引用する。

- **問44（41字）**：Y市に対して、出席停止の懲罰の差止訴訟を提起するとともに、仮の差止めを申し立てる。
- **問45（44字）**：物上代位により、Ｃによる保険金の払渡し前に、Ａが保険金債権を差し押さえなければならない。
- **問46（43字または44字、正解例2通り）**
  - 正解例1：契約不適合責任を根拠に、報酬減額請求、損害賠償請求、契約の解除を主張することができる。（43字）
  - 正解例2：請負人の担保責任を根拠に、報酬減額請求、損害賠償請求、契約の解除を主張することができる。（44字）
